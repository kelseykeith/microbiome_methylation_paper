...
# combine data together and filter for common test sites
lauren_data_wrangled %>%
  filter(sample_id == 'aging') %>%
  select(sma_id, term = sample_id, meth_diff, sig = pvalue) %>%
  rbind(select(lm, sma_id = mm9_SmaI_ID, term, meth_diff = estimate, 
...
# save number and percentage of significant sites in each category in the 
# combined dataset
comb_data %>%
  mutate(sig = case_when(sig < 0.05 & meth_diff < -5 ~ 'hypo',
                         sig < 0.05 & meth_diff > 5 ~ 'hyper',
                         TRUE ~ 'not_sig')) %>%
  group_by(term, sig) %>%
  count() %>%
  ungroup() %>%
  filter(sig != 'not_sig') %>%
  spread(sig, n) %>%
  mutate(perc_hyper = round(((hyper / 12172) * 100), 1),
         perc_hypo = round(((hypo / 12172) * 100), 1)) %>%
  select(term, hypo_count = hypo, perc_hypo, 
         hyper_count = hyper, perc_hyper) %>%
  write_tsv('age_upset_counts_percents.tsv')
```
...
# save counts in table as well
age_upset_hypo %>% 
  group_by(Azoxymethane, IL10KO, Microbiome, Age) %>%
  count() %>%
  ungroup() %>%
  mutate(percent_of_total_sites = round(((n / 12566) * 100), 1)) %>%
# add in the empty intersection that's not present in the table
  rbind(tibble(Azoxymethane = 0, IL10KO = 1, Microbiome = 0, Age = 1, 
               n = 0, percent_of_total_sites = 0)) %>%
# re-sort nicely
  arrange(Azoxymethane) %>%
  write_tsv('age_upset_hypo_counts_percents.tsv')
```
...
age_upset_hyper %>% 
  ...
  ungroup() %>%
  mutate(percent_of_total_sites = round(((n / 12566) * 100), 1)) %>%
  write_tsv('age_upset_hyper_counts_percents.tsv')
```
